...
Yes, it's still there! Let's look at the issue in detail:
...
When you specify issues, only the first six characters are taken into account. For example, issue @tz9540@ may be specified as @tz9540@, @tz9540-colors-are-all-wrong@, or @tz9540-gosh-dang-it-to-heck@. The summary is usually just there to help you recognize the issue, but it may change. The ID remains fixed, however.
...
p. Now it's time to link our issues to some categories. In this case, we already have two types of issues - features and bugs, so let's create categories for them:
...
h3. Link issues to categories

Now we can link issues to categories:

bc. $ ag list
[br9077]   Implement XML export
[tz9540]   Colors are all wrong

bc. $ ag link xml[TAB] feat[TAB]
# this will turn into: ag link br9077-implement-xml-export hc6100-feature

bc. Linked issue br9077-implement-xml-export to 1 category

bc. $ ag link col[TAB] bug[TAB]
# this will turn into: ag link tz9540-colors-are-all-wrong eo6845-bug

bc. Linked issue br9077-implement-xml-export to 1 category

Now the issues are linked to the categories:
...
This gives you all issues which are linked to the 'Bug' category:

bc. [tz9540]   Colors are all wrong (Bug)

As you've seen, categories are arranged in a tree (each category item has a parent, which can be null). Categories are expected to be pretty stable during a project. Issues can be linked to zero, one, or more categories and they're not directly related to each other, they may only share common categories.
...
bc. $ ag start col[TAB]
# this will turn into: ag start bc. $ ag start col[TAB]
...
p. The issue is referenced from the commit via @[tz9540]@. Now let's have a look at the issues again:
...
No you should be able to create issues and categories. Use @ag help@ to learn more about other commands. Once you've set up some categories, you can already specify them on the command line when you create an issue. For example, to file a new bug:

bc. $ ag new bug[TAB]

The newly created issue will be linked to @eo6845-bug@.
