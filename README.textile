...
When you specify issues, only the first six characters are taken into account. For example, issue @tz9540@ may be specified as @tz9540@, @tz9540-colors-are-all-wrong@, or @tz9540-gosh-dang-it-to-heck@. 

While these IDs are really handy to refer to an issue, they're hard to memorize and type. So here's the good news: You can use tab completion for this! To specify issue on the command line, you can type any of the following:

* @tz[TAB]@
* @color[TAB]@
* @wrong[TAB]@

...or even shorter pieces. Ag will expand your input to the issue ID followed by the summary, like this:
...
# We can already specify on the CLI that this category should be a child of 'Issue type', but it could as well be done later using 'ag cat reparent'
