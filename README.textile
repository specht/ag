...
p. The issue is referenced from the commit via @[tz9540]@. How does Ag know we're working on that bug? Ag looks at the branch name. If it starts with two letters and four digits, followed by a dash, it must be an issue! Now let's have a look at the issues again:
