...
The asterisk (*) means that there has already work been done for this issue, that is, there is at least one commit which references the issue. A plus (+) indicates that commits have been made for an issue but in a different branch.
